...
def page_license():
    ...
    st.markdown("""
    設定画面を開くと **左サイドバー** に「🔑 ライセンス」セクションが表示されます。

    「ライセンスキー」欄にコピーしたキーを貼り付け、
    **「🔑 ライセンスを確認」** ボタンをクリックします。
    """)
    img("images/mac/17_license_input.png", "左サイドバーのライセンスキー欄に入力して「ライセンスを確認」をクリック")
    ...
    st.markdown("""
    サイドバー上部に緑色のバッジ「**プラン: AllFull**」が表示されれば認証成功です。
    """)
    img("images/mac/18_license_verified.png", "「プラン: AllFull」が表示されれば認証完了")
...
def page_discord():
    ...
    **ウェブフック** とは、Discord のチャンネルに自動でメッセージを送るための URL です。
    ...
    st.header("ステップ 1: 設定画面の Discord 通知先セクションを確認")

    st.markdown("""
    設定画面の **左サイドバー** を下にスクロールすると、
    「🔔 Discord 通知先（全ツール共通）」セクションがあります。

    ここに Webhook URL を 1 つ入力するだけで、
    **全ツール（新着通知・値下げ通知）の通知がまとめて届きます。**
    """)
    img("images/mac/19_discord_tab.png", "左サイドバーの「Discord 通知先」セクション")
    ...
    > 「せどり通知」などのチャンネルを作ってまとめて受け取るのがおすすめです。
    """)
    ...
    st.markdown("""
    コピーした URL を、左サイドバーの **「Webhook URL」** 欄に貼り付けます。

    貼り付けたら **「📋 通知先を保存」** ボタンをクリックします。
    「保存しました」と表示されれば完了です。
    """)
    img("images/mac/25_webhook_paste.png", "Webhook URL を貼り付けて「通知先を保存」をクリック")
...
def page_mercari():
    ...
    st.header("ステップ 1: メルカリ新着 ページを開く")

    st.markdown("""
    設定画面の上部ナビゲーションから **「🛒 メルカリ新着」** を選択します。

    ページには以下のセクションが表示されます：
    - **起動 / 停止** ― 監視の開始・停止
    - **監視する型番リスト** ― どの商品を監視するかの設定
    """)
    img("images/mac/29_watchlist.png", "「メルカリ新着」ページ")
    ...
    > 💡 **「各列の意味を見る」** をクリックすると、各列の説明が画面上に表示されます。
    """)

    st.divider()

    st.header("ステップ 3: 監視サイクル間隔を設定（任意）")

    st.markdown("""
    左サイドバーの **「新着監視の詳細設定（共通）」** を展開すると、
    監視間隔などの詳細設定ができます。

    | 項目 | 推奨値 | 説明 |
    |---|---|---|
    | 監視サイクル間隔（秒） | 10 | 何秒ごとに監視するか |
    | 並列ワーカー数 | 3 | 同時に監視する型番の数 |
    | 値下げ通知の閾値（%） | 5.00 | 何%値下げされたら通知するか |

    > ⚠️ 監視サイクルを短くしすぎると、メルカリ側からアクセス制限を受けることがあります。

    設定を変更したら **「新着監視の詳細設定を保存」** をクリックします。
    """)
    img("images/mac/27_monitor_settings.png", "左サイドバー「新着監視の詳細設定」")

    st.divider()

    st.header("ステップ 4: 監視を起動する")

    st.markdown("""
    「起動 / 停止」セクションの **「🚀 起動する」** ボタンをクリックします。

    ボタンをクリックすると「🟢 起動中」に変わり、監視が始まります。

    > 📝 停止するには **「⏹ 停止する」** ボタンをクリックしてください。
    """)
    img("images/mac/32_launch.png", "「🚀 起動する」ボタンをクリック")
...
def page_yahoo():
    ...
    設定画面の上部ナビゲーションから **「🟡 ヤフフリ新着」** を選択します。

    「監視する型番リスト」に **ヤフオクフリマの検索結果ページの URL** を入力してください。
    編集後は **「ヤフフリ用監視リストを保存」** ボタンをクリックしてください。

    > 📝 URL はヤフオクのフリマ絞り込みページ（`paypayfleamarket.yahoo.co.jp`）を使ってください。
    """)
    img("images/mac/30_watchlist_yahoo.png", "「ヤフフリ新着」ページ")
...
def page_pricedown():
    ...
    設定後は **「自動値下げの詳細設定を保存」** をクリックしてください。
    """)
    img("images/mac/28_pricedown_settings.png", "左サイドバー「自動値下げの詳細設定」")
    ...
    st.markdown("""
    設定画面の上部ナビゲーションから **「🛒 メルカリ自動値下げ」** を選択します。

    「値下げ対象リスト」に値下げしたい商品を登録します。
    ...
    > 💡 **「🚀 起動する」** ボタンをクリックしてツールを起動すると、
    > 出品中の商品一覧が自動取得されてリストに追加されます。
    """)
    img("images/mac/31_pricelist.png", "「メルカリ自動値下げ」ページ")
...
def page_launch():
    ...
    st.markdown("""
    各ページの上部「**起動 / 停止**」セクションにある **「🚀 起動する」** ボタンをクリックします。

    起動したいツールのページをナビゲーションで選んで、それぞれ起動してください。

    | ページ | 起動されるもの |
    |---|---|
    | 🛒 メルカリ新着 | メルカリの新着監視 |
    | 🟡 ヤフフリ新着 | ヤフフリの新着監視 |
    | 🛒 メルカリ自動値下げ | メルカリの自動値下げ |
    | 🟡 ヤフフリ自動値下げ | ヤフフリの自動値下げ |
    """)
    img("images/mac/32_launch.png", "各ページの「🚀 起動する」ボタンをクリック")

    st.markdown("""
    ボタンをクリックすると、ステータスが **「🟢 起動中」** に変わります。

    > 📝 **停止するには「⏹ 停止する」ボタンをクリックしてください。**
    > ページをリロード・閉じてもバックグラウンドで動き続けます。
    """)
    ...
    st.markdown("""
    各ページの「起動 / 停止」セクションにある **「⏹ 停止する」** ボタンをクリックしてください。
    同じツールを再度「起動する」ボタンで起動すると、前のプロセスが自動的に終了します。
    """)
...
def page_troubleshoot():
    ...
    **確認事項**:
    1. 左サイドバーの「Discord 通知先」に Webhook URL が入力されているか確認
    2. 「通知先を保存」ボタンを押したか確認
    3. Discord のチャンネルで Webhook が有効になっているか確認
    ...
    ```
    Mac: ~/Library/Application Support/sedori-tool-mac/log/
    ```
...
with st.sidebar:
    st.divider()
    st.caption("版: 1.0.76 | 更新: 2026-06-29")
